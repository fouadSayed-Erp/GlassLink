...
fun ChatScreen(messages: List<Message>, onSend: (String)->Unit, onVoice: ()->Unit) {
    ...
    GlassScaffold {
        LazyColumn(Modifier.weight(1f).padding(16.dp)) {
            items(messages) { m ->
                GlassCard(Modifier.fillMaxWidth(0.8f).padding(4.dp)) {
        ...
        Row(Modifier.padding(16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(value=text, onValueChange={text=it}, modifier=Modifier.weight(1f), placeholder={Text("Encrypted message...")})
            GlassButton(Icons.Default.Send, onClick={if(text.isNotBlank()){onSend(text); text=""}})
            GlassButton(Icons.Default.Mic, onClick=onVoice)
        }
    }
}
